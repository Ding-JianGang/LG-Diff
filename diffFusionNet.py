...
class ConditionalFusionNet(nn.Module):

    def __init__(self, img_channel=3, width=32, middle_blk_num=1, enc_blk_nums=[2,2,2], dec_blk_nums=[2,2,2], upscale=1):
        super().__init__()
        ...
        for num in enc_blk_nums:
            self.encoders.append(
                nn.Sequential(
                    *[FusionBlock(chan, time_dim) for _ in range(num)]
                )
            )
            self.downs.append(
        ...
        self.middle_blks = \
            nn.Sequential(
                *[FusionBlock(chan, time_dim) for _ in range(middle_blk_num)]
            )
